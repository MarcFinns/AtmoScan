...
void startProcesses()
{
  ...
  {
#ifdef DEBUG_SYSLOG
    syslog.log(LOG_INFO, "Invalid configuration, starting only UI process");
#endif
  }
    ...
    if (SPIFFS.exists(F("/config.json")))
    {
      ...
      if (configFile)
      {
        ...
        if (json.success())
        {
          ...
          {
            syslog.log(LOG_ERR, "Incompatible configuration found");
            return false;
          }
...
void logESPconfig()
{
  ...
  syslog.log(LOG_DEBUG, "FLASH CONFIGURATION LOG");
  // Log ESP configuration
...
void i2cScan()
{
  ...
  syslog.log(LOG_DEBUG, "Scanning I2C bus...");
